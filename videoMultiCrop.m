...
%     single function capable og encoding all files, with all extensions  is avalibel.
...
FrameRate = ObjInVideo.FrameRate;
...
for iSubMovie=1:nSubVideos
    ...
    if isempty( outVideos{iSubMovie} )
        outVideos{iSubMovie}=[videoPath, filesep, 'subV_', num2str(iSubMovie),...
            videoName, outVideoExt];
    end
    ...
    ObjOutVideo(iSubMovie).VideoWriter.FrameRate = FrameRate;
...
%         imshow(subFrame);
        frameSize = size(subFrame);
        h = frameSize(1);
        w = frameSize(2);
        
        % Resize video according to the smaller dimention
        if (h * 640 >= w * 480) 
            subFrame = imresize(subFrame, [480 NaN]);
        else
            subFrame = imresize(subFrame, [NaN 640]);
        end
...
%         keyboard;
        ...
        subFrame = padarray(subFrame, [480-h 640-w], 255, 'post');
